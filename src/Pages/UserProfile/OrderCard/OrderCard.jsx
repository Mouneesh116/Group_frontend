...
const OrderCard = ({
    ...
    const fetchImageUrl = async () => {
      ...
      try {
        const response = await axios.get(
          `http://16.171.124.12:8000/api/products/getProductImage/${productId}`,
  ...
  const handleCancelOrderItem = async () => {
    ...
    try {
      const response = await axios.post(
        `http://16.171.124.12:8000/api/orders/cancel-item/${order.id}`,
        { productId },
        { headers: { Authorization: `Bearer ${token}` } }
      );
  ...
  const handleReturnRequest = async () => {
    ...
    try {
      const response = await axios.post(
        `http://16.171.124.12:8000/api/orders/request-return/${order.id}`,
        { productId },
        { headers: { Authorization: `Bearer ${token}` } }
      );
  ...
  const handleReviewSubmit = async () => {
    ...
    try {
      const response = await axios.post(
        `http://16.171.124.12:8000/api/users/reviews/add/${productId}`,
